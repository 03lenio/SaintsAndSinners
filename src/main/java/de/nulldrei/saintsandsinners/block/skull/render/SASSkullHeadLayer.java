...
@OnlyIn(Dist.CLIENT)
public class SASSkullHeadLayer<T extends LivingEntity, M extends EntityModel<T>&HeadedModel> extends CustomHeadLayer<T, M> {
    ...
    public SASSkullHeadLayer(RenderLayerParent<T, M>  renderLayerParent, EntityModelSet entityModelSet,
                             ItemInHandRenderer itemInHandRenderer) {
        this(renderLayerParent, entityModelSet, 1.0F, 1.0F, 1.0F, itemInHandRenderer);
    }

    public SASSkullHeadLayer(RenderLayerParent<T, M> renderLayerParent, EntityModelSet entityModelSet, float p_234824_, float p_234825_, float p_234826_, ItemInHandRenderer p_234827_) {
    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int p_116733_, T p_116734_, float p_116735_, float p_116736_, float p_116737_, float p_116738_, float p_116739_, float p_116740_) {
        ...
        if (!itemstack.isEmpty()) {
            ...
            if (item instanceof BlockItem && ((BlockItem)item).getBlock() instanceof AbstractSkullBlock) {
                ...
                SASSkullBlockRenderer.renderSkull(null, 180.0F, animationProgress, poseStack, multiBufferSource, p_116733_, skullmodelbase, rendertype);
            } else {
                label60: {
